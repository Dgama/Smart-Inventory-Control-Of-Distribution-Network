...
if __name__ == '__main__': 
    ...
    # 读取需求
    demand_sample_all = np.loadtxt("demand.txt")
    number_of_sample = int(len(demand_sample_all) / days / len(dc_list))
    demand_sample_all = demand_sample_all.reshape(number_of_sample, days, len(dc_list), len(sku_list)).astype(np.int)

    wanted_days = input("您需要几个样本?(请输入1- " + str(number_of_sample) + "的值)\n ")
    wanted_days = int(wanted_days)
    if wanted_days in list(range(1, number_of_sample + 1)):
        number_of_sample = wanted_days
        demand_sample_all = demand_sample_all[0:wanted_days]
